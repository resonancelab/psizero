...
-- Create admin users view for management
...
SELECT 
  ...
  COUNT(CASE WHEN au.timestamp >= NOW() - INTERVAL '30 days' THEN 1 END) as usage_last_30_days
FROM auth.users u
...
-- Grant access to sysadmins only
GRANT SELECT ON public.admin_users_view TO authenticated;
CREATE POLICY "Only sysadmins can view admin users"
ON public.admin_users_view
FOR SELECT
TO authenticated
USING (public.has_role(auth.uid(), 'sysadmin'));

-- Make first user a sysadmin (insert after they exist)
